...
export function ExperienceSection({ experience }: ExperienceSectionProps) {
  ...
  // Filter out hidden experiences
  const visibleExperience = experience.filter(exp => !exp.isHidden);
  
  // Separate by type
  const workExp = visibleExperience.filter(exp => (exp.type || "work") === "work");
  const otherExp = visibleExperience.filter(exp => exp.type === "other");

  const renderList = (list: PortfolioExperience[]) => (
        ...
        {workExp.length > 0 && (
          <div className="mb-20">
            <h3 className="text-xl font-bold mb-10 text-primary/80 uppercase tracking-widest flex items-center gap-4">
              <span className="h-px flex-grow bg-gradient-to-r from-primary/20 to-transparent"></span>
              {t("admin.experience.type.work")}
              <span className="h-px flex-grow bg-gradient-to-l from-primary/20 to-transparent"></span>
            </h3>
            {renderList(workExp)}
          </div>
        )}
